...
// Skeleton row with fixed widths to prevent hydration mismatch
const SkeletonRow = () => (
  <tr>
    <td><div className="skel" style={{ width: '60%' }} /></td>
    <td><div className="skel" style={{ width: '50%' }} /></td>
    <td><div className="skel" style={{ width: '35%' }} /></td>
...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState<Stats>({ totalAds: 0, pending: 0, approved: 0, rejected: 0, revenue: 0 });
  ...
  useEffect(() => {
    ...
    const parsed: AdminUser = raw ? JSON.parse(raw) : null;
    const isAdmin =
      parsed?.isAdmin === true ||
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Use existing /ads endpoint instead of /admin/ads
        const adsRes = await api.get('/ads');
        const adsData: Ad[] = adsRes.data || [];
        ...
        setStats({
          ...
          approved: adsData.filter(a => a.status === 'approved' || a.status === 'active').length,
        ...
        if (err.code === 'ERR_NETWORK' || err.message?.includes('Network Error')) {
          showToast('⚠️ Backend server is not running. Using demo data. Start backend with: npm run dev', 'error');
        } else {
        ...
        setAds(getDemoAds());
        const demoAds = getDemoAds();
        const revenue = demoAds
          .filter(a => a.status === 'approved' || a.status === 'active')
          .reduce((sum, a) => sum + (a.price || 0), 0);
        setStats({
          totalAds: demoAds.length,
          pending: demoAds.filter(a => a.status === 'pending').length,
          approved: demoAds.filter(a => a.status === 'approved' || a.status === 'active').length,
          rejected: demoAds.filter(a => a.status === 'rejected').length,
          revenue: revenue,
        });
      } finally {
        setLoading(false);
  ...
  const displayedAds = activeTab === 'pending'
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .nav-user{font-size:13px;color:var(--text-soft)}
        ...
        .stats-row{display:grid;grid-template-columns:repeat(5,1fr);gap:16px;margin-bottom:36px;animation:fadeUp .6s ease .1s both}
        @media(max-width:900px){.stats-row{grid-template-columns:repeat(3,1fr)}}
        @media(max-width:580px){.stats-row{grid-template-columns:1fr 1fr}}
        ...
        .stat-value.pending-val{color:#fcd34d}
        .stat-value.approved-val{color:#6ee7b7}
        .stat-value.rejected-val{color:#fca5a5}
        .stat-sub{font-size:12px;color:var(--text-muted);margin-top:6px}

        .tabs{display:flex;gap:4px;background:var(--mid);border:1px solid var(--border);border-radius:12px;padding:4px;width:fit-content;margin-bottom:20px;animation:fadeUp .6s ease .13s both}
        .tab{padding:8px 20px;border-radius:9px;font-size:13px;color:var(--text-muted);cursor:pointer;border:none;background:none;font-family:'DM Sans',sans-serif;transition:all .2s}
        .tab.active{background:var(--mid-2);color:white;box-shadow:0 1px 4px rgba(0,0,0,0.3)}
        .tab-count{display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border-radius:50%;font-size:10px;font-weight:600;margin-left:6px;background:rgba(245,158,11,0.15);color:#fcd34d}
        ...
        .ad-title{color:white;font-weight:400;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        ...
        .btn{padding:7px 14px;border-radius:8px;font-family:'DM Sans',sans-serif;font-size:12px;font-weight:500;cursor:pointer;border:1px solid;transition:all .2s;display:inline-flex;align-items:center;justify-content:center;gap:6px;min-width:80px;text-decoration:none}
        ...
        <div className="inner">
          ...
          <nav className="nav">
            ...
            <div className="nav-right">
              ...
              {adminUser && <span className="nav-user">{adminUser.name}</span>}
              <button
          ...
          {!backendAvailable && (
            <div className="demo-banner">
              ⚠️ Demo Mode: Backend not connected. Using sample data for preview.
            </div>
          )}
          ...
          <div className="stats-row">
            {[
              { label: 'Total Ads', value: stats.totalAds, sub: 'All submissions', cls: '' },
              { label: 'Pending', value: stats.pending, sub: 'Awaiting review', cls: 'pending-val' },
              { label: 'Approved', value: stats.approved, sub: 'Currently live', cls: 'approved-val' },
              { label: 'Rejected', value: stats.rejected, sub: 'Declined ads', cls: 'rejected-val' },
              { label: 'Revenue', value: loading ? '—' : `${stats.revenue.toLocaleString()}`, sub: 'RWF collected', cls: '' },
            ].map(({ label, value, sub, cls }) => (
              <div className="stat-card" key={label}>
                <div className="stat-label">{label}</div>
                <div className={`stat-value ${cls}`}>{loading && label !== 'Revenue' ? '—' : value}</div>
                    ...
                    <td colSpan={7}>
                      <div className="empty">
                        ...
                        <div className="empty-title">
                          {activeTab === 'pending' ? 'All caught up!' : 'No ads yet'}
                        </div>
                        <div className="empty-sub">
                          {activeTab === 'pending'
                            ? 'No ads are waiting for review right now.'
                            : 'No ads have been submitted to the platform yet.'}
                        </div>
                      </div>
                    </td>
                  </tr>
